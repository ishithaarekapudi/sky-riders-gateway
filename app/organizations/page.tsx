...
export default function Organizations() {
  return <PageShell active="organizations">
    ...
    <section className="section" id="organization-directory">
      ...
      <div className="org-grid">{organizations.map(([title,text,tags],i)=><article key={title}><div className="square-icon"><Icon name={i>9?"plane":i>6?"people":"globe"}/></div><div><h3>{title}</h3><p>{text}</p><div className="tag-row">{tags.map(tag=><span key={tag}>{tag}</span>)}</div><SaveButton id={`organization:${title}`} label="Save Organization"/></div></article>)}</div>
