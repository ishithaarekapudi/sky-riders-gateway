...
export default function Organizations({ records }: { records: CatalogItem[] }) {
  ...
  const visible = useMemo(() => records.map(row => [row.title, row.summary, row.tags] as const).filter(([title,text,tags]) => {
