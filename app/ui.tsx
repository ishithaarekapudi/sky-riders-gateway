...
export function Header({ active, originalLogo = false }: { active?: string; originalLogo?: boolean }) {
  const links = [["Home","/"],["Explore","/explore"],["Scholarships","/scholarships"],["Organizations","/organizations"],["Careers","/careers"],["Resources","/resources"]];
  return <header className="site-header">{originalLogo?<BrandLogo />:<Logo />}<nav>{links.map(([label,href])=><Link className={active===label.toLowerCase()?"active":""} href={href} key={href}>{label}</Link>)}<details className={`about-nav-menu${active==="about"||active==="gateway"?" active":""}`}><summary>About</summary><div className="about-nav-dropdown"><Link href="/about/gateway">Gateway</Link><Link href="/about">Ishitha Arekapudi</Link></div></details></nav><div className="account-actions"><Link href="/account" className="ghost-button">Log In</Link><Link href="/account" className="small-button">Sign Up</Link></div><MobileNav active={active}/></header>;
}
