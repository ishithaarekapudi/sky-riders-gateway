...
export function Header({ active, originalLogo = false }: { active?: string; originalLogo?: boolean }) {
  const links = [["Scholarships","/scholarships"],["Organizations","/organizations"],["Careers","/careers"],["Resources","/resources"]];
  return <header className="site-header">{originalLogo?<BrandLogo />:<Logo />}<nav><Link className={active==="home"?"active":""} href="/">Home</Link><Link className={active==="explore"?"active":""} href="/explore">Explore</Link><AboutMenu active={active}/>{links.map(([label,href])=><Link className={active===label.toLowerCase()?"active":""} href={href} key={href}>{label}</Link>)}</nav><div className="account-actions"><Link href="/account" className="ghost-button">Log In</Link><Link href="/account" className="small-button">Sign Up</Link></div><MobileNav active={active}/></header>;
}
