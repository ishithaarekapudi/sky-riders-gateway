...
export default function Careers({ records }: { records: CatalogItem[] }) {
  const slugify = (title: string) => records.find(row => row.title === title)?.slug || "";
  const [query, setQuery] = useState("");
  const [filter, setFilter] = useState<(typeof careerFilters)[number]>("All Careers");
  const careerPaths = records.map(row => [row.icon, row.title, row.summary] as const);
  const groups = [...families.map(family => ({ ...family, titles: records.filter(row => row.category ? row.category === family.label : (family.titles as readonly string[]).includes(row.title)).map(row => row.title) })), { icon: "plane", prompt: "Explore more paths", label: "More Career Paths", note: "Discover additional aviation and aerospace pathways.", titles: records.filter(row => !families.some(family => row.category ? row.category === family.label : (family.titles as readonly string[]).includes(row.title))).map(row => row.title) }];
  const visibleFamilies = groups.filter(family => family.titles.length).filter(family => filter === "All Careers"
  ...
  return <PageShell active="careers">
    ...
    <section className="section careers-directory" id="top">
      ...
      <nav className="career-family-nav" aria-label="Career interests">{groups.filter(family => family.titles.length).map((family)=><a href={`#career-family-${groups.indexOf(family)}`} key={family.label}><Icon name={family.icon}/><span>{family.prompt}</span><strong>{family.label}</strong><small>See {family.titles.length} careers →</small></a>)}</nav>
      {visibleFamilies.map((family)=><section className="career-family" id={`career-family-${groups.indexOf(family)}`} key={family.label}>
        <header><div><span>{family.prompt}</span><h2>{family.label}</h2><p>{family.note}</p></div><a href="#top">Back to choices ↑</a></header>
        <div className="career-grid organization-style-career-grid">{careerPaths.filter(([,title,text])=>family.titles.includes(title) && `${title} ${text} ${family.label}`.toLowerCase().includes(query.toLowerCase())).map(([icon,title,text])=><article className="directory-profile-card career-profile-card" key={title}><SaveButton id={`career:${title}`} label={title}/><div className="directory-profile-copy"><div className="career-card-heading"><div className="career-brand-icon">{records.find(row => row.title === title)?.logoUrl ? <img className="directory-managed-logo" src={records.find(row => row.title === title)!.logoUrl} alt="Organization logo"/> : <Icon name={icon}/>}</div><span className="organization-type">{family.label}</span></div><h3><Link href={`/careers/${slugify(title)}`}>{title}</Link></h3><p>{text}</p><div className="tag-row"><span>Career guide</span><span>Skills & training</span><span>Next steps</span></div><div className="card-actions"><Link href={`/careers/${slugify(title)}`}>Explore this career →</Link></div></div></article>)}</div>
      </section>)}
