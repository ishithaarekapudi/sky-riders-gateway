...
export default function Scholarships({ records }: { records: CatalogItem[] }) {
  ...
  const visible = useMemo(() => records.map(row => [row.icon, row.title, row.award || "See official source", row.tags.join(" · ")] as const).filter(([,title,,tags]) => {
  ...
  return <PageShell active="scholarships">
    ...
    <section className="directory scholarship-directory runway-directory">
      ...
      <div className="scholar-grid runway-scholar-grid">{visible.map(([,title,amount,tags])=><article className="scholar-card runway-scholar-card directory-profile-card" key={title}>
        <div className="scholar-copy directory-profile-copy">{records.find(row => row.title === title)?.logoUrl && <img className="directory-managed-logo" src={records.find(row => row.title === title)!.logoUrl} alt="Organization logo" loading="lazy"/>}<span className="scholar-type">{records.find(row => row.title === title)?.info.sourceLabel || sponsor(title)}</span><h3><Link href={`/scholarships/${slugify(title)}`}>{title}</Link></h3><p className="scholarship-card-intro">{records.find(row => row.title === title)?.summary}</p><div className="runway-scholar-facts"><div><span>Award</span><strong>{amount}</strong></div><div><span>Eligibility</span><b>{tags}</b></div><div><span>Application</span><b>Verify the current official cycle</b></div></div><div className="card-actions"><Link href={`/scholarships/${slugify(title)}`}>View scholarship →</Link></div></div>
