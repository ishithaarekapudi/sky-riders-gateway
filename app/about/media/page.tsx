...
export const metadata: Metadata = withPageSeo({ title: "Ishitha Arekapudi Media and Press", description: "Explore selected interviews and media features about Ishitha Arekapudi, aviation, aerospace, youth access, and Sky Riders Gateway.", alternates: { canonical: "/about/media" } });
...
export default async function MediaPage() {
  ...
  return <PageShell active="about">
    ...
    <section className="section media-logo-section">
      ...
      <div className="media-logo-wall">{mediaLogos.map(outlet => <article key={outlet.id}>{outlet.websiteUrl ? <a href={outlet.websiteUrl} target="_blank" rel="noreferrer"><img src={outlet.logoUrl} alt={`${outlet.name} logo`} /></a> : <img src={outlet.logoUrl} alt={`${outlet.name} logo`} />}<span>{outlet.name}</span></article>)}</div>
    </section>
